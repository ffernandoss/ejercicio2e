...
fun ListaCompraScreen(modifier: Modifier = Modifier) {
    ...
    // Función para añadir un producto a Firebase
    fun addProductoToFirebase(producto: Producto) {
        db.collection("listaCompra")
            .add(producto)
            .addOnSuccessListener {
                toastMessage = "Producto añadido"
                showToast = true
            }
            .addOnFailureListener {
                toastMessage = "Error al añadir producto"
                showToast = true
            }
    }

    // Función para eliminar un producto de Firebase
    fun removeProductoFromFirebase(producto: String) {
        db.collection("listaCompra")
            .whereEqualTo("producto", producto)
            .get()
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    db.collection("listaCompra").document(document.id).delete()
                }
                toastMessage = "Producto eliminado"
                showToast = true
            }
            .addOnFailureListener {
                toastMessage = "Error al eliminar producto"
                showToast = true
            }
    }
    ...
    Column(
        ...
        Button(onClick = {
            when {
                ...
                listaCompra.any { it.producto == producto } -> {
                    ...
                    addProductoToFirebase(nuevoProducto)
                    producto = ""
                    cantidad = ""
                    precio = ""
                }
        ...
        Button(onClick = {
            ...
            if (productoEncontrado != null) {
                ...
                removeProductoFromFirebase(productoAEliminar)
            } else {
